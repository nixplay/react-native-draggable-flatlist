...
class SortableFlatList extends Component {
  ...
  animate = () => {
    const { activeRow } = this.state;
    const {
      scrollPercent, data, scrollSpeed, // horizontal,
    } = this.props;
  ...
  getSpacerIndex = (activeRow) => {
    ...
    const spacerIndex = this._measurements.findIndex(({
      width, height, x, y,
    }) => (hoverPointX > x && hoverPointX < (x + width))
  ...
  moveStart = (evt, hoverComponent, index) => {
    ...
    if (Platform.OS === 'android' && !horizontal) {
      const isTranslucent = StatusBar._propsStack.reduce(((acc, cur) => (cur.translucent === undefined ? acc : cur.translucent)), false);

      const isHidden = StatusBar._propsStack.reduce(((acc, cur) => (cur.hidden === null ? acc : cur.hidden.value)), false);
  ...
  renderItem = ({ item, index }) => {
    ...
    const { activeRow, spacerIndex, extraData } = this.state;
  ...
  renderHoverComponent = () => {
    ...
    return !!hoverComponent && (
      <Animated.View style={[
        ...
        { transform: [{ translateX: this._hoverAnimX }, { translateY: this._hoverAnimY }, { scale: 0.8 }] },
      ]}
      >
        {hoverComponent}
      </Animated.View>
...
class RowItem extends PureComponent {
  ...
  render() {
    const {
      moveEnd, isActiveRow, horizontal, renderItem, item, index, setRef, style,
    } = this.props;
    ...
    return (
      <View ref={setRef(index)} collapsable={false} style={[{ opacity: 1, flexDirection: horizontal ? 'row' : 'column' }, style]}>
